...
function calculateSubtotal() {
	...
	for(let i = 0; i < ITEMS.length; i++){
		quantity = parseInt(document.getElementById(ITEMS[i].concat("-quantity")).value);
		price = parseFloat(document.getElementById(ITEMS[i].concat("-price")).innerText);
		result += quantity*price;
	}
	return result; // TODO calculateSubtotal
}

function calculateSalesTax() {
	return roundMoney(calculateSubtotal()*getSalesTaxRateForState(document.getElementById("state-tax").value)); // TODO calculateSalesTax
}
